...
async function waitForPreviewServer(port, timeoutMs = 30000) {
  const url = `http://localhost:${port}`;
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    if (await isServerResponding(url, 2000)) {
      return port;
    }
...
function startPreviewServer(projectPath, port, framework = 'vite') {
  return new Promise(async (resolve, reject) => {
    ...
    function tryNext() {
      ...
      const resolveStarted = (actualPort) => {
        if (started) return;
        started = true;
      ...
      // HTTP health check works even when stdout pipe is lost after parent process exits
      waitForPreviewServer(port, 30000).then((readyPort) => {
        if (readyPort) resolveStarted(readyPort);
      });
...
async function restorePreviewServer(deploymentId) {
  ...
  const checkUrl = `http://localhost:${deployment.port}`;
  if (await isServerResponding(checkUrl)) {
    return { deploymentId, subdomain: deployment.subdomain, port: deployment.port, action: 'already_running' };
  }
  ...
  const projectPath = projectInfo.type === 'project' || projectInfo.type === 'nextjs-built'
    ? projectInfo.path
    : path.dirname(projectInfo.path);

  if (framework === 'nextjs') {
    const hasNext = await fs.pathExists(path.join(projectPath, '.next'));
    const hasOut = await fs.pathExists(path.join(projectPath, 'out'));
    if (!hasNext && !hasOut) {
      throw new Error('Next.js build output missing — full rebuild required');
    }
  } else {
    const hasDist = await fs.pathExists(path.join(projectPath, 'dist'));
    if (!hasDist && projectInfo.type !== 'dist') {
      throw new Error('Vite dist folder missing — full rebuild required');
    }
  }

  try {
    if (deployment.pid) {
      try { process.kill(-deployment.pid, 'SIGTERM'); } catch (_) { /* ignore */ }
    }
    if (deployment.port) {
      execSync(`lsof -ti:${deployment.port} | xargs kill -9 2>/dev/null`, { stdio: 'ignore' });
    }
  ...
  const port = deployment.port;
  console.log(`[RESTORE] Starting preview for ${deployment.subdomain} on port ${port} (no rebuild)`);

  const { process: previewProcess, pid, port: actualPort } = await startPreviewServer(projectPath, port, framework);
  const serverPort = actualPort || port;
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(pid || previewProcess.pid, serverPort, deploymentId);

  previewProcess.unref();

  return { deploymentId, subdomain: deployment.subdomain, port: serverPort, action: 'restored' };
}
...
async function ensurePreviewServersRunning() {
  ...
  console.log(`[RESTORE] Checking ${rows.length} running deployment(s)...`);
  ...
  for (const row of rows) {
    ...
    console.log(`[RESTORE] Preview server down for ${row.subdomain} (port ${row.port})`);
    try {
      await restorePreviewServer(row.id);
      restored++;
    } catch (restoreErr) {
      console.log(`[RESTORE] Fast restore failed for ${row.subdomain}: ${restoreErr.message}, trying full restart...`);
      try {
        await restartDeployment(row.id);
        restored++;
      } catch (restartErr) {
        failed++;
        console.error(`[RESTORE] Failed to restart ${row.subdomain}: ${restartErr.message}`);
      }
    }
  }

  console.log(`[RESTORE] Complete: ${healthy} healthy, ${restored} restored, ${failed} failed, ${rows.length} total`);
  return { checked: rows.length, healthy, restored, failed };
}
