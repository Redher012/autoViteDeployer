    ...
    function tryNext() {
      ...
      childProcess.on('close', (code) => {
        if (code === 0) {
          ...
          if (code === 127) {
            console.log(`${cmd} not found (exit code 127), trying next package manager...`);
            currentCmd++;
            if (currentCmd < commands.length) {
              return tryNext();
            }
...
async function buildProject(projectPath) {
  return new Promise(async (resolve, reject) => {
    ...
    function tryNext() {
      ...
      // Check if command exists before trying - use full path resolution
      let fullCmd = cmd;
      try {
        const whichOutput = execSync(`which ${cmd} 2>/dev/null || command -v ${cmd}`, { 
          encoding: 'utf8',
        ...
        // Command not found, try next
        console.log(`${cmd} not found in PATH, trying next package manager for build...`);
        currentCmd++;
        return tryNext();
      }
