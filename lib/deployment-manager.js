...
async function buildProject(projectPath) {
  return new Promise(async (resolve, reject) => {
    ...
    // Determine command order based on lock files
    // If vite exists, add npx vite build as a fallback option
    let commands = [];
    if (hasPnpmLock) {
      commands = [
        { cmd: 'pnpm', args: ['run', 'build'] },
        { cmd: 'npm', args: ['run', 'build'] }, // Fallback to npm if pnpm not available
        ...(viteExists ? [{ cmd: 'npx', args: ['vite', 'build'] }] : []) // Fallback to npx vite if vite exists
      ];
    } else if (hasYarnLock) {
      commands = [
      ...
      // Default: try npm first, then pnpm, then npx vite if vite exists
      commands = [
        { cmd: 'npm', args: ['run', 'build'] },
        { cmd: 'pnpm', args: ['run', 'build'] },
        ...(viteExists ? [{ cmd: 'npx', args: ['vite', 'build'] }] : [])
      ];
    }
      ...
      childProcess.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          if (isNpmNotFound) {
            ...
            // A script command (like vite) wasn't found - try next command in list
