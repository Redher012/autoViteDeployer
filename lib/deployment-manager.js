...
async function captureScreenshot(url, deploymentId, subdomain) {
  try {
    ...
    const hasContent = await page.evaluate(() => {
      ...
      return body && (body.textContent.trim().length > 0 || body.children.length > 0);
    });
    ...
    // Get page background color to diagnose black screen
    const bgColor = await page.evaluate(() => {
      return window.getComputedStyle(document.body).backgroundColor;
    });
    console.log(`[SCREENSHOT] Page background color: ${bgColor}`);
