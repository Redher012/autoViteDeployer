...
async function buildProject(projectPath) {
  return new Promise(async (resolve, reject) => {
    ...
    function tryNext() {
      ...
      const nodeModulesBinExists = await fs.pathExists(nodeModulesBin);
      ...
      childProcess.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          if (isNpmNotFound) {
            ...
            // A script command (like vite) wasn't found - this is a build error, not npm error
            console.log(`[BUILD] Script command not found in build script (exit code 127)`);
            console.log(`[BUILD] Full output: ${output.substring(0, 500)}`);
            // Don't try next package manager - this is a project-specific issue
