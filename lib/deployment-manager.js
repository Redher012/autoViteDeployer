...
// #region agent log
const DEBUG_LOG_PATH = path.join(process.cwd(), '.cursor', 'debug-d58f2e.log');
function _dbgLog(location, message, data, hypothesisId) {
  const payload = { sessionId: 'd58f2e', location, message, data, hypothesisId, timestamp: Date.now(), runId: 'post-fix' };
  fetch('http://127.0.0.1:7869/ingest/1a39c01b-3591-4172-8246-bbc1fffdaeb7', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': 'd58f2e' }, body: JSON.stringify(payload) }).catch(() => {});
  try {
    fsNative.mkdirSync(path.dirname(DEBUG_LOG_PATH), { recursive: true });
    fsNative.appendFileSync(DEBUG_LOG_PATH, JSON.stringify(payload) + '\n');
  } catch (_) { /* ignore */ }
}
function _findParentLockfiles(projectPath) {
  const found = [];
  ...
  while (dir !== root) {
    for (const name of ['package-lock.json', 'pnpm-lock.yaml', 'yarn.lock']) {
      ...
      if (fsNative.existsSync(lockPath)) found.push(lockPath);
    }
...
async function buildProject(projectPath, framework = 'vite') {
  return new Promise(async (resolve, reject) => {
    ...
    // #region agent log
    let pkgBuildScript = null;
    let pkgNextVersion = null;
    try {
      const pkg = await fs.readJson(path.join(projectPath, 'package.json'));
      pkgBuildScript = pkg.scripts?.build || null;
      pkgNextVersion = (pkg.dependencies?.next || pkg.devDependencies?.next) || null;
    } catch (_) { /* ignore */ }
    const parentLockfiles = _findParentLockfiles(projectPath);
    _dbgLog('deployment-manager.js:buildProject:entry', 'buildProject called', {
      framework, projectPath, pkgBuildScript, pkgNextVersion, parentLockfiles,
      deployerCwd: process.cwd(), hasWebpackFlagInBuildScript: pkgBuildScript?.includes('--webpack') ?? false
    }, 'A,B,C,D,E');
    // #endregion
    ...
    // #region agent log
    _dbgLog('deployment-manager.js:buildProject:commands', 'build commands prepared', {
      framework,
      commands: commands.map(c => ({ cmd: c.cmd, args: c.args, hasWebpackFlag: c.args.includes('--webpack') })),
      anyCommandHasWebpack: commands.some(c => c.args.includes('--webpack'))
    }, 'A,C');
    // #endregion
    ...
    function tryNext() {
      ...
      // #region agent log
      _dbgLog('deployment-manager.js:buildProject:spawn', 'spawning build command', {
        cmdIndex: currentCmd, fullCmd, args, cwd: projectPath,
        hasWebpackFlag: args.includes('--webpack'), framework
      }, 'A,C');
      // #endregion
      ...
      childProcess.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          // #region agent log
          _dbgLog('deployment-manager.js:buildProject:fail', 'build command failed', {
            cmd, args, code, cmdIndex: currentCmd,
            hasTurbopackError: output.includes('Turbopack') || output.includes('turbopack'),
            hasWebpackConfigError: output.includes('webpack') && output.includes('config'),
            hasLockfileWarning: output.includes('lockfile') || output.includes('lockfiles'),
            hasWebpackFlag: args.includes('--webpack'),
            outputSnippet: output.trim().substring(0, 800)
          }, 'A,B,C,D');
          // #endregion
          currentCmd++;
          if (currentCmd < commands.length) {
